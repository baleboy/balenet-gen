...
When I developed my first iOS application using AI, I just described to the agent how I wanted it to look like, using words like "minimalistic" and "easy to use". The result was a standard SwiftUI application that looked nice but also generic. I also used words to describe the first version of my (this) website, and that worked better. In general AI agents seem to do better at web design.

![Transport app](transport-app-1st-version.jpeg)

## Copy with shame

In some video I saw somebody describing the following workflow:

* Look for a design you like on [Dribbble](https://dribbble.com)
* Take screenshots, give them to an AI agent and tell it to create a design system
...
Last week Google released a new version of [Stitch](https://stitch.withgoogle.com), a design tool similar to Figma that got some new AI capabilities. Basically you give it a prompt and it creates a design that you can then edit and give to a coding agent via MCP. I used it again for my pizza app and it created very nice designs that look like a high end recipe app. The screens are backed by HTML which makes it easy to the coding agent to parse and understand. I also used it to redesign my website, which it did excellently. The only thing it did poorly was designing a lock screen widget for iOS.
...
## Give AI the ability to see

I haven't yet given Claude Code the ability to see the screenshots produced by the iOS code. So far I've resorted to taking screenshots myself and passing them to the agent. A better way would be to connect it with XCode MCP, but for that I need Xcode 26.3, which needs MacOS 26 which... based on my past experience with OS updates, I don't date to install.
